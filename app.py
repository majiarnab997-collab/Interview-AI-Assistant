...
================================================
Railway-ready deployment file (no Colab dependency).
...
# ============================================================
# 1. Startup — Environment Variable Validation (Railway-native)
...
if missing_vars:
    raise EnvironmentError(
        ...
        f"Add them in Railway under Project -> Service -> Variables."
    )
...
# ------------------------------------------------------------
# Persistence note:
# Railway's container filesystem is EPHEMERAL — it resets on every
# redeploy/restart. A plain relative path here (the old bug) means
# your interview history quietly disappears.
#
# To actually persist data on Railway:
#   1. Attach a Railway Volume to this service (Service -> Settings -> Volumes),
#      mount it at e.g. /data
#   2. Set an env var:  HISTORY_FILE_PATH=/data/mock_interview_history.json
# If HISTORY_FILE_PATH isn't set, it falls back to a local file, which is
# fine for local testing but will NOT survive a Railway redeploy.
# ------------------------------------------------------------
HISTORY_FILE_PATH = os.environ.get("HISTORY_FILE_PATH", "mock_interview_history.json")
...
# ============================================================
# 9. Long-Term Memory
#    Persists to HISTORY_FILE_PATH. On Railway, point this at a
#    mounted Volume (see note near the top of this file) or the
...
with gr.Blocks(title="Interview AI Assistant") as demo:
    ...
    gr.Markdown(
        ...
        "multi-agent mock interview panel."
    )
...
if __name__ == "__main__":
    # Railway assigns the port dynamically via the PORT env var and expects
    # the app to bind to 0.0.0.0. `share=True` (a temporary public gradio.live
    # link) is unnecessary and unreliable in a container — Railway already
    # gives you a public URL for the service.
    demo.launch(
        server_name="0.0.0.0",
